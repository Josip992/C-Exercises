...
{
	// kreira novu hash tablicu (alocira samu strukturu i niz pokazivača)
...
{
	//sta je s provjerom da li je count= size tj da je tablica puna
	...
	ht->table[key] = tmp; //dodjela adrese liste
}
...
int Get(HashTable* ht, char* word)
{
	...
	while (item != NULL)
	{
		...
		{
			return 1;      //main ga negira na 0
		}
		item = item->next;
	}
	return 0;       //main ga negira na 1
}
